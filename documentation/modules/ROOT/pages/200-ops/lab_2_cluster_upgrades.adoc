...
[NOTE]
====
Depending on when you run this command the available version list may be longer than just the next release. Or it may have no available upgrades available. In that case you are already on the latest release and can not upgrade your cluster any further. Please read through the rest of these instructions and then move on to the next lab.
====
