...
----
+
The *ID* of the MachinePool that you want to add autoscaling to is `Default` - note that in your environment it may be called something different like `workers`.
...
. It will take a few minutes (like 5) for the new nodes to be available.
Check that the `MachinePool` is provioning new nodes.
+
[source,sh]
----
rosa list machinepools --cluster rosa-$GUID
...
workers  Yes          2-4               2                 m5.xlarge                          us-east-2a         subnet-0b83b72575eb742dc  4.12.35  Yes                         WaitingForAvailableMachines: NodeProvisioning  
----
+
You should see the text `WaitingForAvailableMachines: NodeProvisioning` to indicate that new nodes are being brought online.

. Check the number of nodes in your cluster (repeat this command until you see 4 nodes - the maximum that you configured for autoscaling the Machinepool):
+
[source,sh]
----
oc get nodes
